...
## Terragrunt Usage in terragrunt.hcl file
...
locals {
  ...
  country = local.account_vars.inputs.country
...
dependency "mongodbatlas-cluster" {
    config_path = "../../../common/mongodbatlas/cluster"
    mock_outputs = {
        mongodbatlas_aws_containers = {"${local.primary_region}" = {"atlas_cidr_block" : "10.0.0.0/16"},"${local.secondary_region}" = {"atlas_cidr_block" : "11.0.0.0/16"}} #Sample CIDR for mock output
    }
}
...
inputs = {
    name        = "${local.country}-${local.env}-lambda-security-group"
    path        = "/"
    description = "${local.country} ${local.env} ${local.region} security group for lambda"
    vpc_id = dependency.vpc.outputs.vpc_id

    ingress_cidr_blocks = [lookup(dependency.mongodbatlas-cluster.outputs.mongodbatlas_aws_containers[local.primary_region],"atlas_cidr_block"),lookup(dependency.mongodbatlas-cluster.outputs.mongodbatlas_aws_containers[local.secondary_region],"atlas_cidr_block")]
    ingress_rules = ["mongodb-27017-tcp"]

    egress_cidr_blocks = [lookup(dependency.mongodbatlas-cluster.outputs.mongodbatlas_aws_containers[local.primary_region],"atlas_cidr_block"),lookup(dependency.mongodbatlas-cluster.outputs.mongodbatlas_aws_containers[local.secondary_region],"atlas_cidr_block")]
    egress_rules = ["mongodb-27017-tcp"]
}
```
